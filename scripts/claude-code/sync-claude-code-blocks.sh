...
if [ -n "$CLAUDE_SYNC_TEMP_DIR" ]; then
    SOURCE_DIR="$CLAUDE_SYNC_TEMP_DIR/templates/blocks"
else
    SOURCE_DIR="$PROJECT_ROOT/templates/blocks"
fi
# Removed old path
    ...
    if [ -f "$block_file" ]; then
        ...
        if [[ "$first_line" == "---" ]]; then
            ...
            PROJECT_ROOT="$PROJECT_ROOT" awk '
                ...
                in_frontmatter && /^---$/ && NR > 1 { 
                    print; 
                    system("cat " ENVIRON["PROJECT_ROOT"] "/templates/blocks/block-command-block.md");
                    print "";
                    print "````````````";
                    in_frontmatter = 0; 
                    found_end = 1; 
            ...
            {
                cat "$PROJECT_ROOT/templates/blocks/block-command-block.md"
                echo ""
                echo "\`\`\`\`\`\`\`\`\`\`"
                cat "$block_file"
                echo "\`\`\`\`\`\`\`\`\`\`"
