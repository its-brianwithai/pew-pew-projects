...
echo "📋 Processing wikilinks in copied prompt files..."

# Process all copied prompt files to replace wikilinks
find "$CLAUDE_COMMANDS_PLX_DIR" -name "*.md" -type f -exec bash -c '
    file="$1"
    temp_file=$(mktemp)
